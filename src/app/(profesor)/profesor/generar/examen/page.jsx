...
export default function GenerarExamen() {
  ...
  const [vistaTab, setVistaTab] = useState('con_respuestas')
  ...
  // Load questions from biblioteca evaluacion
  useEffect(() => {
    if (searchParams.get('desde') !== 'biblioteca' || inicializado.current) return
    const raw = sessionStorage.getItem('kleo_evaluacion_enviar')
    if (!raw) return
    inicializado.current = true
    sessionStorage.removeItem('kleo_evaluacion_enviar')
    try {
      const data = JSON.parse(raw)
      if (data.preguntas?.length) {
        setTareaGenerada(data.preguntas)
        setForm((prev) => ({
          ...prev,
          nombre: data.nombre || prev.nombre,
          materia: data.materia || prev.materia,
          numeroPreguntas: data.preguntas.length,
        }))
        // Auto-save as draft
        if (profesor?.id && clases?.[0]?.id) {
          agregarTarea.mutateAsync({
            profesor_id: profesor.id,
            clase_id: clases[0].id,
            nombre: data.nombre || 'Evaluacion',
            materia: data.materia || 'Matemáticas',
            dificultad: 'Media',
            metodologia: 'Práctica directa',
            tipos: ['Ejercicio mixto'],
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <NavBar titulo={tareaGenerada ? 'Revisar borrador' : 'Generar examen'} volver="/profesor/generar" />
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8 animate-fade-in">
        ...
        {!tareaGenerada ? (
          <div className="space-y-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 mb-1">Nuevo examen con IA</h1>
          ...
          <div className="animate-fade-in space-y-6">
            ...
            {/* Tabs: Sin respuestas / Con respuestas */}
            <div className="flex rounded-xl bg-gray-100 p-1">
              <button
                type="button"
                onClick={() => setVistaTab('sin_respuestas')}
                className={`flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-all ${
                  vistaTab === 'sin_respuestas'
              ...
              <button
                type="button"
                onClick={() => setVistaTab('con_respuestas')}
                className={`flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-all ${
                  vistaTab === 'con_respuestas'
                    ? 'bg-white text-gray-900 shadow-sm'
                    : 'text-gray-500 hover:text-gray-700'
                }`}
              >
            ...
            {vistaTab === 'con_respuestas' ? (
              ...
              <VistaExamen preguntas={tareaGenerada} nombre={form.nombre} />
            )}
            ...
            {/* Enviar a alumnos */}
            {clases.length > 0 && (
              <div className="card p-6 space-y-4 border-2 border-amarillo">
                <div>
                  <h3 className="font-semibold text-gray-900 mb-1 flex items-center gap-2">
                    <svg className="w-5 h-5 text-amarillo" viewBox="0 0 20 20" fill="currentColor">
                      <path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z" />
                    </svg>
                    Enviar a alumnos
                  </h3>
                  <p className="text-xs text-gray-400 mb-4">
                    Una vez enviado, el examen será visible para los alumnos de las clases
                    seleccionadas.
                  </p>
                  {clases.length > 1 && (
                    ...
                    {publicando ? (
                      ...
                      <>
                        ...
                        {clasesPublicar.length > 1
                          ? `Enviar a ${clasesPublicar.length} clases`
                          : 'Enviar a alumnos'}
                      </>
                    )}
                  </Boton>
...
function VistaExamen({ preguntas, nombre }) {
  return (
    <div className="space-y-3">
      <div className="card p-6 border-b-2 border-gray-200">
        <h2 className="text-lg font-bold text-gray-900 mb-1">{nombre || 'Examen'}</h2>
        ...
        <div key={i} className="card p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {p.tipo === 'espacios' && (
                <div className="mt-3 border-b-2 border-dotted border-gray-300 w-48 h-6" />
              )}

              {(p.tipo === 'abierta' || p.tipo === 'calculo') && (
                <div className="mt-3 space-y-2">
                  {[1, 2, 3].map((n) => (
                    <div key={n} className="border-b border-gray-200 h-6" />
                  ))}
                </div>
              )}
            </div>
