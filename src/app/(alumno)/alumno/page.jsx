...
export default function DashboardAlumno() {
  ...
  function promedioPersonal() {
    ...
    const suma = conNota.reduce((a, t) => a + (t.resultadoAlumno.calificacion_manual ?? t.resultadoAlumno.calificacion), 0)
            ...
            {tareas.map((tarea) => {
              ...
              return (
                <div
                  ...
                  onClick={() => puedeRealizar && !vencida && router.push(`/alumno/tarea/${tarea.id}`)}
                >
                        ...
                        {fechaLimite && (
                          <>
                            ...
                            <span className={vencida ? 'text-red-500' : ''}>
                              Límite: {fechaLimite.toLocaleDateString('es-MX', { day: 'numeric', month: 'short' })}
                            </span>
                          </>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      {completada && tarea.resultadoAlumno?.calificacion != null && (
                        <span className={`text-2xl font-bold ${
                          (tarea.resultadoAlumno.calificacion_manual ?? tarea.resultadoAlumno.calificacion) >= 8
                            ? 'text-green-600'
                            : (tarea.resultadoAlumno.calificacion_manual ?? tarea.resultadoAlumno.calificacion) >= 6
                            ? 'text-orange-500'
                            : 'text-red-500'
                        }`}>
                          {tarea.resultadoAlumno.calificacion_manual ?? tarea.resultadoAlumno.calificacion}<span className="text-sm font-normal text-gray-400">/10</span>
                        </span>
                      )}
                      {puedeRealizar && !vencida && (
                        <Boton
                          variante="primario"
                          size="sm"
                          onClick={(e) => { e.stopPropagation(); router.push(`/alumno/tarea/${tarea.id}`) }}
                        >
                          Realizar
                        </Boton>
                      )}
                      {completada && (
                        <Boton
                          variante="secundario"
                          size="sm"
                          onClick={(e) => { e.stopPropagation(); router.push(`/alumno/resultado/${tarea.id}`) }}
                        >
                          Ver resultado
                        </Boton>
                      )}
