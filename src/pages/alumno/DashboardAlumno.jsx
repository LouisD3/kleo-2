...
export default function DashboardAlumno() {
  ...
  const { getTareasParaAlumno } = useTareaStore()

  useEffect(() => {
    if (!perfilActivo || !alumnoSeleccionado) navigate('/')
  }, [perfilActivo, alumnoSeleccionado, navigate])

  if (!alumnoSeleccionado) return null

  const tareas = getTareasParaAlumno(alumnoSeleccionado.id)
  ...
  function promedioPersonal() {
    ...
    const suma = conNota.reduce((a, t) => a + t.resultadoAlumno.calificacion, 0)
          ...
          <div className="space-y-3">
            {tareas.map((tarea) => {
              ...
              return (
                <div
                  key={tarea.id}
                  className={`card p-5 transition-all ${puedeRealizar ? 'hover:shadow-md cursor-pointer' : ''}`}
                  onClick={() => puedeRealizar && navigate(`/alumno/tarea/${tarea.id}`)}
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-3 text-xs text-gray-500">
                        ...
                        <span>·</span>
                        <span>{tarea.tipos.slice(0, 2).join(', ')}{tarea.tipos.length > 2 ? '...' : ''}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0">
                      {completada && tarea.resultadoAlumno?.calificacion != null && (
                        <span className={`text-2xl font-bold ${
                          tarea.resultadoAlumno.calificacion >= 8
                            ? 'text-green-600'
                            : tarea.resultadoAlumno.calificacion >= 6
                            ? 'text-orange-500'
                            : 'text-red-500'
                        }`}>
                          {tarea.resultadoAlumno.calificacion}<span className="text-sm font-normal text-gray-400">/10</span>
                        </span>
                      )}
                      {puedeRealizar && (
                        <Boton
                          variante="primario"
                          size="sm"
                          onClick={(e) => { e.stopPropagation(); navigate(`/alumno/tarea/${tarea.id}`) }}
                        >
                          Realizar
                        </Boton>
                      )}
                      {completada && (
                        <Boton
                          variante="secundario"
                          size="sm"
                          onClick={(e) => { e.stopPropagation(); navigate(`/alumno/resultado/${tarea.id}`) }}
                        >
                          Ver resultado
                        </Boton>
                      )}
